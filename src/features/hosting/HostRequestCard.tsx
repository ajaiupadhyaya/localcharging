...
import { notifyBooking } from '@/lib/payments/stripe';
...
export function HostRequestCard({ booking }: HostRequestCardProps) {
  ...
  const approve = async () => {
    ...
    if (error) Alert.alert('Error', error.message);
      ...
      notifyBooking(booking.id, 'booking_approved').catch(() => undefined);
  ...
  const decline = async () => {
    ...
    if (error) Alert.alert('Error', error.message);
      ...
      notifyBooking(booking.id, 'booking_declined').catch(() => undefined);
      qc.invalidateQueries({ queryKey: ['host-requests'] });
    }
  };
  ...
  return (
    <View style={styles.card}>
      ...
      <Text style={styles.body}>{booking.profiles?.display_name ?? 'Driver'} wants to charge</Text>
      <Text style={styles.meta}>
        Est. ${booking.estimated_cost?.toFixed(2)} · ~{booking.requested_kwh} kWh
      </Text>
      {booking.driver_message ? <Text style={styles.message}>"{booking.driver_message}"</Text> : null}
